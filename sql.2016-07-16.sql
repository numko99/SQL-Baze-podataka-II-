...
*/
CREATE DATABASE zadatak_2016_IB180063
ON PRIMARY 
(
	NAME='zadatak_2016_IB180063',
	FILENAME='C:\Program Files\Microsoft SQL Server\MSSQL15.MSSQLSERVER\MSSQL\DATA\zadatak_2016_IB180063.mdf',
	SIZE=5 MB,
	MAXSIZE = UNLIMITED,
	FILEGROWTH=10%
)
LOG ON 
(
NAME='zadatak_2016_IB180063_log',
	FILENAME='C:\Program Files\Microsoft SQL Server\MSSQL15.MSSQLSERVER\MSSQL\DATA\zadatak_2016_IB180063_log.ldf',
	SIZE=5 MB,
	MAXSIZE = UNLIMITED,
	FILEGROWTH=10%
)
USE zadatak_2016_IB180063
GO
...
Mail NVARCHAR(50) NOT NULL UNIQUE NONCLUSTERED,
...
(
	TransakcijaID INT CONSTRAINT PK_Transakcija_id PRIMARY KEY IDENTITY(1,1),
	Datum DATETIME NOT NULL,
	TipTransakcije NVARCHAR(30) NOT NULL,
	PosiljalacID INT CONSTRAINT FK_PosiljacalID FOREIGN KEY (PosiljalacID) REFERENCES KLIJENT(KlijentID),
	PrimalacID INT CONSTRAINT FK_PrimalacID FOREIGN KEY (PrimalacID) REFERENCES KLIJENT(KlijentID),
	Svrha NVARCHAR(30) NOT NULL,
	Iznos DECIMAL(18,2)
)
...
*/
	INSERT INTO KLIJENT(Ime,Prezime,Telefon,Mail,BrojRacuna,KorisnickoIme,Lozinka)
	SELECT TOP 10 P.FirstName,P.LastName,PH.PhoneNumber,E.EmailAddress,C.AccountNumber,P.FirstName+'.'+P.LastName AS KorisnickoIme,RIGHT(PASS.PasswordHash,8)
				...
				INNER JOIN AdventureWorks2014.Person.EmailAddress AS E ON E.BusinessEntityID=P.BusinessEntityID
...
ON KLIJENT(Ime,Prezime)
INCLUDE(BrojRacuna)

SELECT Ime,Prezime from KLIJENT
where BrojRacuna Like 'AW%'

ALTER INDEX  ix_Ime_Prezime ON KLIJENT
DISABLE
...
*/
	CREATE PROCEDURE procedura_upis_klijenata
	(
		@ime NVARCHAR(30),@prezime NVARCHAR(30),@telefon NVARCHAR(20),@mail NVARCHAR(50),@brojRacuna NVARCHAR(15),@korisnickoIme NVARCHAR(20),@lozinka NVARCHAR(20)
	)
	AS 
	BEGIN
	INSERT INTO KLIJENT
	...
	EXEC procedura_upis_klijenata 'admir','numanovic','062-745-148','admir_numanovic@hotmail.com','123123123','admir.numanovic','asdasdgsd'
	select * from KLIJENT
/*5.
...
*/
	CREATE VIEW viewTransakcija
	as
	select Datum,TipTransakcije,KPosljiaoc.Ime+' ' + KPosljiaoc.Prezime as Posiljaoc,KPosljiaoc.BrojRacuna as BrojRacunaPosiljaoca
	,KPrimaoc.Ime+ ' ' +KPrimaoc.Prezime as Primaoc,KPrimaoc.BrojRacuna as BrojRacunaPrimaoca,Svrha,Iznos
	from TRANSAKCIJE AS T INNER JOIN KLIJENT AS KPosljiaoc ON T.PosiljalacID=KPosljiaoc.KlijentID 
														INNER JOIN KLIJENT AS KPrimaoc ON T.PrimalacID=KPrimaoc.KlijentID
	select * from viewTransakcija
/*6.
...
procedure
*/
CREATE PROCEDURE procedura_broj_racuna
(@brojRacuna NVARCHAR(20))
AS
BEGIN
select * from viewTransakcija WHERE BrojRacunaPosiljaoca=@brojRacuna
END
 EXEC procedura_broj_racuna 'AW00011001'
SELECT * FROM viewTransakcija
...
*/
SELECT DATEPART(YEAR,Datum),SUM(Iznos) FROM TRANSAKCIJE
...
*/
	...
	CREATE PROCEDURE procedura_briisanja_podataka
	(
	@Klijentid INT
	)
	as
	BEGIN
	DELETE FROM TRANSAKCIJE WHERE (PosiljalacID=@Klijentid OR PrimalacID=@Klijentid)
	DELETE FROM KLIJENT WHERE (KlijentID=@Klijentid)
	END
	EXEC procedura_briisanja_podataka 1
	SELECT * FROM KLIJENT
	SELECT * FROM TRANSAKCIJE
/*9.
...
ALTER PROCEDURE procedura_brojRacuna_prezime
(@prezime nvarchar(30)=NULL,
@brojRacuna NVARCHAR(30)=NULL
)
AS
BEGIN
	SELECT * FROM viewTransakcija WHERE 
	(BrojRacunaPosiljaoca=@brojRacuna OR @brojRacuna IS NULL ) 
	AND (SUBSTRING(Posiljaoc,CHARINDEX(' ',Posiljaoc)+1,30)=@prezime OR @prezime IS NULL)
	
	
END
EXEC procedura_brojRacuna_prezime
EXEC procedura_brojRacuna_prezime @brojRacuna='AW00011001'
EXEC procedura_brojRacuna_prezime @prezime='Huang'
EXEC procedura_brojRacuna_prezime @prezime='Huang',@brojRacuna='AW00011001'
	SELECT SUBSTRING(Posiljaoc,CHARINDEX(' ',Posiljaoc),30) FROM viewTransakcija
/*10. Napraviti full i diferencijalni backup baze podataka na default lokaciju servera*/

BACKUP DATABASE zadatak_2016_IB180063 TO
DISK='zadatak_2016_IB180063.bak'

BACKUP DATABASE zadatak_2016_IB180063 TO
DISK='zadatak_2016_IB180063'
WITH DIFFERENTIAL